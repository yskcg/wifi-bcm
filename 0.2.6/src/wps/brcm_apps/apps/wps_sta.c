...
found:
	...
	if (pbc ==  WPS_UI_PBC_HW) {
		...
		if (atoi(wps_ui_get_env("wps_method")) == WPS_UI_METHOD_PBC) {
			...
			*/
			if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR && sta_wksp->configap == false) {
...
wpssta_wksp_start(wpssta_wksp_t *sta_wksp)
{
	...
	if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
		...
		if (wpssta_start_registration(pin, start_time) != WPS_SUCCESS) {
		...
		if (wpssta_start_enrollment(pin, start_time) != WPS_SUCCESS) {
...
wpssta_do_protocol_again(wpssta_wksp_t *sta_wksp)
{
	...
	if (sta_wksp->sc_mode == SCMODE_STA_REGISTRAR) {
		ret = wpssta_start_registration(pin, current_time);
	}
	else {
		ret = wpssta_start_enrollment(pin, current_time);
	}
