 ...
 *
 * $Id: ap_api.c 296937 2011-11-17 06:35:35Z kenlo $
 */
...
MSS:
	...
	if (!dummy_ssid_applied && info->b_mca) {
	...
	if (wps_get_mode(g_mc) == SCMODE_AP_ENROLLEE) {
		if (g_mc->mp_enrSM) {
			...
			if (START == regInfo->e_smState &&
			    memcmp(regInfo->dev_info->pre_nonce,
				empty_enrolleeNonce, SIZE_128_BITS)) {
