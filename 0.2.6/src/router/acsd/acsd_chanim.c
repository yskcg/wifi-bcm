 ...
 *
 * $Id: acsd_chanim.c 241396 2011-02-18 03:39:46Z stakita $
 */
...
chanim_chk_lockout(chanim_info_t *ch_info)
{
	...
	start_idx = MODSUB(cur_idx, chanim_config(ch_info).max_acs, CHANIM_ACS_RECORD);
	start_record = &ch_info->record[start_idx];

	/* don't count the first one (init scan) */
...
chanim_scb_lastused(acs_chaninfo_t* c_info)
{
	...
	ret = wl_iovar_getint(c_info->name, "scb_lastused", &lastused);
