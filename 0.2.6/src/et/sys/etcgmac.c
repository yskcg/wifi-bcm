...
chipduplexupd(ch_t *ch)
{
	...
	link = 9;
	link = 1 << link;
	status = chipphyrd(ch, 2, 1);
	if (!linkstate && (status & (1 << 5))) {
		linkstate = TRUE;
		si_gpioout(ch->sih, link, 0, GPIO_DRV_PRIORITY);
		si_gpioouten(ch->sih, link, link, GPIO_DRV_PRIORITY);
	}
	else if(linkstate && !(status & (1 << 5))){
		linkstate = FALSE;
		si_gpioout(ch->sih, link, link, GPIO_DRV_PRIORITY);
		si_gpioouten(ch->sih, link, link, GPIO_DRV_PRIORITY);
	}
