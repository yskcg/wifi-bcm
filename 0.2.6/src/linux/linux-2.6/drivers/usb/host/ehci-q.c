...
)
{
	...
	{
		ehci_err(ehci, "EHCI Fastpath: Attempted non-optimized write to optimized pipe\n");
		return qh;
	}
...
static void start_unlink_async (struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	if (ehci_optimized(ehci, qh) >= 0)
	{
		ehci_err(ehci, "EHCI Fastpath: Regular unlink of optimized pipe\n");
	}
...
rescan:
	...
	if (likely (qh != NULL)) {
		do {
			/* Optimized path first */
			if (qh->stamp != ehci->stamp) {
				int pipeindex = ehci_optimized(ehci, qh);
				if (pipeindex >= 0  &&
				    ehci->ehci_bypass_callback != NULL) {
					qh->stamp = ehci->stamp;
					ehci->ehci_bypass_callback(pipeindex, qh, &ehci->lock);			
					goto next;
				}	
